...
def config():
    ...
    parser.add_argument("--version", type=str, default='phase2_v4.3_2000_dist_returns')  
    ...
    parser.add_argument("--phase1_version", type=str, default='phase1_v4.0')
    ...
    parser.add_argument("--partner_update_rate", type=float, default=0.7) 
...
class Train():
    ...
    def train(self):
        ...
        for episode in range(cfg.s_episode, cfg.e_episode + 1):
            ...
            win_count = board.count(1)
            win_rate = win_count / len(board)
            print(f'Win Rate {win_rate:.2f}, Board {board}')

            if win_rate >= cfg.partner_update_rate and len(self.buffer) >= cfg.start_update: 
